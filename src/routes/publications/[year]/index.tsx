...
function classify(publications: Publication[]): {
  ...
  const internationals: Map<InternationalSubcategory, Publication[]> =
    new Map();
...
function translateInternationalSubcategory(
  sub: InternationalSubcategory,
): string {
  switch (sub) {
    case "conference":
      return "国際会議";
...
export default component$(() => {
  ...
  const { journals, internationals, domestics } = classify(
    publicationsPerYear.value,
  );
  ...
  return (
    <main>
      ...
      <section>
        <h2>国際発表</h2>
        {[...internationals.entries()].map(([h3, pubs]) => (
          <section key={h3}>
...
export const onStaticGenerate: StaticGenerateHandler = async () => {
  ...
  return {
    params: [...years.values()].map((year) => ({ year: year.toString() })),
  };
};
